...
export default {
	async fetch(request, env, ctx) {
		...
		if (Router.isWebSocketUpgrade(request) && url.pathname === "/ax_ws") {
...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/setup-password" && request.method === "POST") {
			...
			return new Response(JSON.stringify({ success: true }), {
				headers: {
					...
					"Set-Cookie": "panel_session=" + hashed + "; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=259200",
				},
			});
		}
		if (url.pathname === "/api/login" && request.method === "POST") {
			const { password, captcha_answer, hash, timestamp } = await request.json();
			const now = Date.now();
			if (!timestamp || !hash || !captcha_answer || now - parseInt(timestamp) > 5 * 60 * 1000) {
				return new Response(JSON.stringify({ error: "کپچا منقضی شده است. لطفا دوباره تلاش کنید" }), {
					status: 400, headers: { "Content-Type": "application/json; charset=utf-8" },
				});
			}
			const storedHash = await DbService.getPanelPassword(env.DB);
			const expectedHash = await DbService.sha256(captcha_answer + storedHash + timestamp);
			if (expectedHash !== hash) {
				return new Response(JSON.stringify({ error: "کپچا اشتباه است" }), {
					status: 400, headers: { "Content-Type": "application/json; charset=utf-8" },
				});
			}
			...
			if (storedHash === hashedInput) {
				return new Response(JSON.stringify({ success: true }), {
					headers: {
						"Content-Type": "application/json; charset=utf-8",
						"Set-Cookie": "panel_session=" + storedHash + "; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=259200",
					},
				});
			}
		...
		if (url.pathname === "/api/captcha" && request.method === "GET") {
			...
			const hash = await DbService.sha256(num + storedHash + timestamp);
			const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="120" height="40" viewBox="0 0 120 40">
				<rect width="120" height="40" fill="#f0f0f0"/>
				<text x="60" y="28" font-family="monospace" font-size="24" font-weight="bold" fill="#333" text-anchor="middle" letter-spacing="4">${num}</text>
				<line x1="0" y1="10" x2="120" y2="30" stroke="#ccc" stroke-width="2"/>
				<line x1="0" y1="30" x2="120" y2="10" stroke="#aaa" stroke-width="1.5"/>
			</svg>`;
			return new Response(JSON.stringify({ svg, hash, timestamp }), {
				headers: { "Content-Type": "application/json; charset=utf-8" },
			});
		}
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/aavsvs80-coder/axon_net/main/core.js?t=" + Date.now() + Math.random(), {
					headers: {
				...
				formData.append("ax.js", new Blob([newCode], { type: "application/javascript+module" }), "ax.js");
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				const githubRes = await fetch("https://raw.githubusercontent.com/aavsvs80-coder/axon_net/main/core.js?t=" + Date.now(), {
					headers: {
						"Cache-Control": "no-cache, no-store, must-revalidate",
						Pragma: "no-cache",
						Expires: "0",
		...
		if (url.pathname === "/api/change-password" && request.method === "POST") {
			...
			return new Response(JSON.stringify({ success: true }), {
				headers: {
					"Content-Type": "application/json; charset=utf-8",
					"Set-Cookie": "panel_session=" + newHash + "; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=259200",
				},
			});
		}
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2Fax_ws#" + encodeURIComponent(m1));
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2Fax_ws#" + encodeURIComponent(m2));
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + host + ":80?path=%2Fax_ws&security=none&encryption=none&host=" + host + "&fp=" + fp + "&type=ws#" + encodeURIComponent(infoRemark));
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + ip + ":" + portStr + "?path=%2Fax_ws&security=" + tlsVal + "&encryption=none&insecure=0&host=" + host + "&fp=" + fp + "&type=ws&allowInsecure=0&sni=" + host + userFrag + "#" + encodeURIComponent(remark));
			});
		});
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    ...
    <script>
        ...
        window.alert = function(message) {
            ...
            if (msgStr.includes('خطا') || msgStr.includes('') || msgStr.includes('')) {
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        <div id="login-section">
            ...
            <form onsubmit="handleLogin(event)" class="space-y-4">
                ...
                <div class="flex flex-col gap-2">
                    <label class="block text-sm font-medium">کد امنیتی (کپچا)</label>
                    <div class="flex items-center gap-2">
                        <div id="captcha-img" class="cursor-pointer border border-gray-300 rounded overflow-hidden" onclick="loadCaptcha()" title="تغییر کپچا"></div>
                        <input type="text" id="captcha-input" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-center font-mono" required placeholder="عدد را وارد کنید">
                    </div>
                    <input type="hidden" id="captcha-hash">
                    <input type="hidden" id="captcha-timestamp">
                </div>
        ...
        <div id="recovery-section" class="hidden">
            ...
            <div class="mb-5 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-xl text-xs leading-relaxed text-orange-800 dark:text-orange-300">
                ...
                <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=ax-Deployer-Token" target="_blank" class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-900/20 hover:text-green-800 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-900/40 dark:hover:text-green-400 rounded-lg font-bold transition shadow-md">
    ...
    <script>
        ...
        async function loadCaptcha() {
            try {
                ...
                document.getElementById('captcha-hash').value = data.hash;
                document.getElementById('captcha-timestamp').value = data.timestamp;
            } catch(e) {}
        }
...
<div class="pt-2 border-t border-gray-100 dark:border-zinc-900">
    ...
    <div class="mt-4 p-3 bg-gray-50/50 dark:bg-zinc-900/20 border border-gray-200/60 dark:border-zinc-800 rounded-2xl shadow-sm">
        <div class="flex items-center gap-1.5 mb-2">
            ...
            <span class="text-[11px] font-bold text-green-700 dark:text-green-500"> پورت‌های دلخواه (با فاصله جدا کنید)</span>
        </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-150 dark:border-zinc-900 space-y-4">
                        ...
                        <div class="pt-4 border-t-2 border-gray-200 dark:border-zinc-800">
                            <div class="flex items-center justify-between gap-2 mb-3">
                                ...
                                <a href="https://github.com/aavsvs80-coder/axon_net" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-bold shadow-sm flex-shrink-0">ساخت پروکسی شخصی</a>
                            </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-xl text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div id="token-modal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="token-modal-card" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-6 transform transition-all scale-95 opacity-0 duration-200">
            ...
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=ax-Deployer-Token" target="_blank" class="flex items-center justify-center gap-2 w-full py-3 bg-[#d94800] hover:bg-[#e35802] text-white font-bold rounded-xl text-sm transition duration-300 mb-4 shadow-md shadow-orange-500/20">
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    ...
    <script>
        ...
        async function bulkDelete() {
            ...
            if (await customConfirm(' آیا از حذف گروهی ' + usernames.length + ' کاربر انتخاب شده مطمئن هستید؟ این عمل غیرقابل بازگشت است.')) {
                const bar = document.getElementById('bulk-actions-bar');
        ...
        async function handleBulkEditSubmit(event) {
            ...
            if (!applyLimit && !applyExpiry && !applyReqLimit && !applyIpLimit && !applyFingerprint && !applyPorts && !applyIps && !applyFrag) {
                alert(' لطفا حداقل یک فیلد را برای اعمال تغییر انتخاب کنید!');
        ...
        function renderUsersUI(data) {
            try {
                ...
                if (cfRequests >= 90000) {
                    ...
                    const today = new Date().toISOString().split('T')[0];
                    if (localStorage.getItem('ax_usage_warned_date') !== today) {
                        openUsageWarning();
                    }
                } else {
                    if (reqCard) {
        ...
        async function handleFormSubmit(event) {
            ...
            if (checkedPorts.length === 0) {
                alert(' لطفا حداقل یک پورت را برای اتصال انتخاب کنید!');
                submitButton.disabled = false;
...
function closePathWarning() {
    ...
    card.classList.remove('opacity-100', 'scale-100');
    card.classList.add('opacity-0', 'scale-95');
    localStorage.setItem('ax_path_warned_' + CURRENT_VERSION, 'true');
}
function closeUsageWarning() {
    ...
    card.classList.add('opacity-0', 'scale-95');
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('ax_usage_warned_date', today);
}
...
function getVlessLink(username) {
            ...
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    ...
                    const tlsVal = isTlsPort ? 'tls' : 'none';
                    const remark = user.username + ' | \u200E' + ip + ' | \u200E' + portStr;
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fax_ws&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
...
window.filterLocations = function() {
    ...
    try {
        ...
        async function exportUsersBackup() {
            if (!window.allUsers || window.allUsers.length === 0) {
                alert(' کاربری برای پشتیبان‌گیری وجود ندارد!');
                return;
            }
            try {
                ...
                downloadAnchor.setAttribute("download", "ax_full_backup_" + dateStr + ".json");
            ...
            reader.onload = async function(e) {
                ...
                try {
                    ...
                    if (backupSettings && Object.keys(backupSettings).length > 0) {
                        const restoreSettings = await customConfirm(' فایل بک‌آپ شامل تنظیمات پنل نیز می‌باشد. آیا می‌خواهید تنظیمات هم بازگردانی شوند؟');
                        if (restoreSettings) {
                            try {
                    ...
                    if (duplicates.length > 0) {
                        overwrite = await customConfirm(' تعداد ' + duplicates.length + ' کاربر تکراری شناسایی شد. آیا می‌خواهید اطلاعات آن‌ها بازنویسی شود؟');
                    }
        ...
        async function changeAdminPassword() {
            ...
            if (!currentPwd || !newPwd) {
                alert(' وارد کردن رمز عبور فعلی و جدید الزامی است!');
                return;
            }
            if (newPwd.length < 4) {
        ...
        async function logoutAdmin() {
			if (await customConfirm('آیا می‌خواهید از پنل خارج شوید؟  ')) {
                try {
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/aavsvs80-coder/axon_net/main/core.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/aavsvs80-coder/axon_net/main/ips.txt');
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (localStorage.getItem('ax_path_warned_' + CURRENT_VERSION) !== 'true') {
                const modal = document.getElementById('path-warning-modal');
                const card = modal.querySelector('div');
                modal.classList.remove('opacity-0', 'pointer-events-none');
                modal.classList.add('opacity-100', 'pointer-events-auto');
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('ax_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پنل تغییر کرد');
            };

            const savedRate = localStorage.getItem('ax_refresh_rate');
...
async function fetchAndLoadProxy() {
    ...
    try {
		const sources = [
			...
			{ url: 'https://raw.githubusercontent.com/aavsvs80-coder/axon_net/main/proxy/' + country.toUpperCase() + '.txt', prefix: '' },
        ...
        <div class="border-t border-gray-100 dark:border-zinc-800 pt-6 relative z-10">
            ...
            <div class="space-y-3">
                <button onclick="copyTextSub()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-indigo-500 dark:hover:border-indigo-500 rounded-xl text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2"> کپی لینک ساب‌اسکریپشن متنی</span>
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fax_ws&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
