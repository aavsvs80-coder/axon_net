	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/aavsvs80-coder/axon_net/refs/heads/main/net.js?t=" + Date.now() + Math.random(), {
					headers: {
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				const githubRes = await fetch("https://raw.githubusercontent.com/aavsvs80-coder/axon_net/refs/heads/main/net.js?t=" + Date.now(), {
					headers: {
						"Cache-Control": "no-cache, no-store, must-revalidate",
						Pragma: "no-cache",
						Expires: "0",
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/aavsvs80-coder/axon_net/refs/heads/main/net.js?t=' + Date.now());
